...
bool inside(const Point& point, const std::vector<Point>& points) {
    Point prev = points[points.size() - 1];
    Point extra = Point(101273, 102611);
    int cnt = 0;
    for (size_t ind = 0; ind < points.size(); ++ind) {
        if (point.x == points[ind].x && point.y == points[ind].y) {
            return true;
        }
    }
    for (size_t ind = 0; ind < points.size(); ++ind) {
        if (side(prev, points[ind], point) == 0 && point.x <= std::max(prev.x, points[ind].x) && point.x >= std::min(prev.x, points[ind].x) && point.y <= std::max(prev.y, points[ind].y) && point.y >= std::min(prev.y, points[ind].y)) {
            return true;
        }
        prev = points[ind];
    }
    for (size_t ind = 0; ind < points.size(); ++ind) {
        if (cross({ point, extra, prev, points[ind] })) {
            ++cnt;
        }
        prev = points[ind];
    }
    for (size_t ind = 0; ind < points.size(); ++ind) {
        if (side(point, extra, points[ind]) == 0 && point.x < points[ind].x) {
            --cnt;
        }
        prev = points[ind];
    }
